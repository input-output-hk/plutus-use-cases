<template>
  <div>
    <b-sidebar id="main-sidebar" title="My Wallets" bg-variant="dark" text-variant="light" shadow>
      <div class="px-3 py-2">
        ...
        <b-container>
          <b-row v-for="(token,index) in balance.tokens" :key="index"
            ...
            <b-col class="col-8 py-1">
              ...
              <strong class="text-lg-left"> : </strong>{{ token.name }}
            </b-col>
        ...
        <b-modal size="lg" :id="''+tokenClicked.index" title="Sell Token" ref="my_modal" hide-footer>
          <b-form @submit.prevent="onSell()">
            <div class="py-1"><strong>Policy : </strong><span class="text-sm-left">{{ tokenClicked.currency }}</span>
            </div>
