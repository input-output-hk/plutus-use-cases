...
import           Control.Lens
import           Contracts.PubKey (AsPubKeyError (..), PubKeyError)
import qualified Contracts.PubKey as PK
...
type CurrencySchema =
    BlockchainActions
        .\/ Endpoint "createNativeToken" SimpleMPS
...
forgeCurrency = do
    SimpleMPS{tokenName, amount} <- endpoint @"createNativeToken"
