...
requestHandler configs _httpManager _pool = RequestHandler $ \case
  ...
  Api_EstimateSwap tokenAInput -> estimateSwapOutput tokenAInput
      ...
      case mPoolStateSymbol of
        ...
        Just poolStateSymbol -> do
          ...
          case mCurrentDatumFile of
            Nothing -> return $ Left "ob-backend: Pool Datum not found"
            Just currentDatumFile -> do
              ...
              recentPoolBalanceBytes <- LBS.readFile $ (T.unpack swapProposalDir) <> "/recentPoolBalance.json"
...
estimateSwapOutput tokenAInput = do
