...
getFeesConstraints :: forall s. NftAppSymbol -> NftId -> Integer -> Contract UserWriter s Text ([Constraints.TxConstraints UserAct DatumNft], [Constraints.ScriptLookups NftTrade])
getFeesConstraints symbol nftId price = do
  user <- getUId
